...
const Navbar=()=>{
    ...
    useEffect(()=>{
        const handleClickOutside=(event)=>{
            ...
            if(mobileCreateMenuRef.current && !mobileCreateMenuRef.current.contains(event.target)){
    ...
    const renderUserActions = (isMobile = false) => {
        const isMoreOpenVal = isMobile ? isMobileMoreOpen : isMoreOpen;
        const setIsMoreOpenVal = isMobile ? setIsMobileMoreOpen : setIsMoreOpen;
        const moreMenuRefVal = isMobile ? mobileMoreMenuRef : moreMenuRef;

        const isCreateOpenVal = isMobile ? isMobileCreateOpen : isCreateOpen;
        const setIsCreateOpenVal = isMobile ? setIsMobileCreateOpen : setIsCreateOpen;
        const createMenuRefVal = isMobile ? mobileCreateMenuRef : createMenuRef;

        return (
            <>
                { user &&
                <div className="relative" ref={moreMenuRefVal}>
                    <button
                        type="button"
                        onClick={()=>setIsMoreOpenVal((v)=>!v)}
                ...
                {!loading && user ?(
                    <>
                        <div className="relative" ref={createMenuRefVal}>
                            <button
                                type="button"
                                onClick={()=>setIsCreateOpenVal((v)=>!v)}
                                className="flex items-center gap-1.5 hover:bg-zinc-800 rounded-full px-3 py-2 text-zinc-300 hover:text-white transition text-sm font-medium border border-zinc-700 md:border-transparent"
                            >
                                <Plus size={16}/>
                                <span>Create</span>
                            </button>
    ...
    return (
        <header className="relative md:sticky md:top-0 z-50 bg-zinc-950/90 backdrop-blur-md border-b border-zinc-800">
            <div className="flex flex-col md:flex-row md:items-center justify-between px-4 py-3 md:px-6 gap-3 md:gap-4">
                
                {/* Row 1 for Mobile: logo + user controls */}
                ...
                {/* Row 2 for Mobile / Center for Desktop: Search bar */}
                <form onSubmit={handleSearch} className="w-full md:flex-1 md:max-w-2xl mx-0 md:mx-4">
